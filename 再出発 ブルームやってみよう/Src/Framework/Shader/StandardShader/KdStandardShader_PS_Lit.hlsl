...
float3 ExtractBrightPass(float3 color)
{
	...
	if (brightness > g_BloomThreshold) // 高輝度抽出の閾値
	{
		return color * g_BloomIntensity; // ブルーム強度を適用
	}
	return float3(0, 0, 0);
}

float3 ApplyGaussianBlur(float2 uv)
{
	float2 offsets[9] =
	{
	...
	for (int i = 0; i < 9; i++)
	{
		blurredColor += g_baseTex.Sample(g_ss, uv + offsets[i] * 0.002).rgb * weights[i];
	}
...
float4 main(VSOutput In) : SV_Target0
{
    ...
    // 高輝度部分の抽出
	float3 brightPassColor = ExtractBrightPass(baseColor.rgb);

    // ガウスぼかし（簡略版：周辺ピクセルの平均値を取る）
	float2 offsets[5] =
	{
		float2(-0.002, 0), float2(0.002, 0), float2(0, -0.002), float2(0, 0.002), float2(0, 0)
	};

	float3 blurredColor = float3(0, 0, 0);
	for (int i = 0; i < 5; i++)
	{
		blurredColor += g_baseTex.Sample(g_ss, In.UV + offsets[i]).rgb;
	}
	blurredColor /= 5;

    // ベース色とブルームを合成
	//outColor = baseColor.rgb + blurredColor;
