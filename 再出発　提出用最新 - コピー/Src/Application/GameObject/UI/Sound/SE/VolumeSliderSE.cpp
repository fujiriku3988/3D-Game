...
void VolumeSliderSE::DrawSprite()
{
	//青いスライドだけに適用
	//切り取り位置、切り取り範囲
	...
	if (m_drawFlg == true)
	{
		...
		KdShaderManager::Instance().m_spriteShader.DrawTex(&m_handleTex, m_handlePos.x, m_handlePos.y,
			m_handleTexSize.x * m_handleScale.x, m_handleTexSize.y * m_handleScale.y, nullptr, &m_color);
	}
}

void VolumeSliderSE::Update()
{
	//マウス座標
	POINT nowPos;
	//マウス座標取得
	...
	KdAudioManager::Instance().UpdateSEVolume(volume);

	m_blueDrawAdjust.x = m_handlePos.x - m_blueTexSizeHarf.x;
}

void VolumeSliderSE::WirteJsonFile()
{
