...
void Fence::Init(const std::string _filePath, int _id)
{
	...
	m_rot = JsonManager::Instance().GetParamVec3(_filePath, m_objName, "rot");
...
void Fence::Update()
{
	...
	Math::Matrix rotMatX = Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(m_rot.x));
	Math::Matrix rotMatY = Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(m_rot.y));
	Math::Matrix rotMatZ = Math::Matrix::CreateRotationY(DirectX::XMConvertToRadians(m_rot.z));
	Math::Matrix rotMat = rotMatX * rotMatY * rotMatZ;
