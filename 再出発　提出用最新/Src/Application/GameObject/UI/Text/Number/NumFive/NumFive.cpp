...
void NumFive::Init(const std::string _filePath)
{
	m_tex.Load("Asset/Textures/UI/Text/number.png");
	m_pos = JsonManager::Instance().GetParamVec2(_filePath, "NumFive", "pos");
	m_scale = JsonManager::Instance().GetParamVec2(_filePath, "NumFive", "scale");
	m_texSize = JsonManager::Instance().GetParamVec2(_filePath, "NumFive", "texSize");
	m_texSizeHarf = JsonManager::Instance().GetParamVec2(_filePath, "NumFive", "texSizeHarf");
	m_anim = JsonManager::Instance().GetParamVec2(_filePath, "NumFive", "anim");
	m_color = JsonManager::Instance().GetParamVec4(_filePath, "NumFive", "color");
	m_drawFlg = JsonManager::Instance().GetParam<bool>(_filePath, "NumFive", "drawFlg");
}

void NumFive::DrawSprite()
{
...
void NumFive::Update()
{
		...
		&& nowLength.x >= m_pos.x - m_texSizeHarf.x && nowLength.x <= m_pos.x + m_texSizeHarf.x)
	{
		...
		if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
		{
			if (m_key == false)
			{
				Fade::Instance().BootBlackFade(SceneManager::SceneType::Stage5);
				m_key = true;
			}
		}
		else
