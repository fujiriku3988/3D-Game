...
POINT JsonManager::GetParamPOINT(const std::string& _filePath, const std::string& _name, const std::string& _key)
{
	std::ifstream file(_filePath);
	if (!file.is_open()) {
		throw std::runtime_error("Error: Could not open file: " + _filePath);
	...
	if (jsonData.contains(_name) && jsonData[_name].contains(_key)) {
		auto& posData = jsonData[_name][_key];
		POINT result;
		result.x = posData.value("x", 0); //デフォルト値を 0.0f に設定
		result.y = posData.value("y", 0);
		return result;
	}
	else
	{
