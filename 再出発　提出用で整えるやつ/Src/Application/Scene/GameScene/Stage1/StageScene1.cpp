...
{
	//ステージ切り替え時エフェクト初期化（最初のマップだけど念のため）
	...
	goalPoint->Init("Asset/Data/Json/Stage/Stage1/GoalPoint/GoalPoint.json",goalID++);
	AddObject(goalPoint);
	//==================================================//

	//プレイヤー
	std::shared_ptr<Player> player = std::make_shared<Player>();
	player->Init("Asset/Data/Json/Player/Stage1/Player.json");
	AddObject(player);
