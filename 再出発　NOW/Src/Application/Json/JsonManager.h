...
public:
	...
	//POINT型のデータを取ってくるよう
	POINT GetParamPOINT(const std::string& _filePath, const std::string& _name, const std::string& _key);
...
inline T JsonManager::GetParam(std::string _filePath, std::string _name, std::string _key)
{
	...
	{
		// ファイルが開けなかった場合
		throw std::runtime_error("Error: Failed to open file '" + _filePath + "'.");
	}
}
...
template <typename T>
inline T JsonManager::AddParam(const std::string& _filePath, const std::string& _name, const std::string& _key, const T& _value)
{
	std::ifstream file(_filePath);
	json jsonData;
