...
public:
	...
	// 位置を取得する
	virtual Math::Vector3 GetPosition() const { return m_pos; }
	// スケールを取得する
	virtual Math::Vector3 GetScale() const { return m_scale; }
	// 位置を設定する（オブジェクトの移動）
	virtual void SetPosition(const Math::Vector3& pos) { m_pos = pos; }
	// スケールを設定する（オブジェクトのサイズ変更）
	virtual void SetScale(const Math::Vector3& scale) { m_scale = scale; }
