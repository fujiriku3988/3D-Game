...
void NumThree::Init(const std::string _filePath)
{
	m_tex.Load("Asset/Textures/UI/Text/number.png");
	m_pos = JsonManager::Instance().GetParamVec2(_filePath, "NumThree", "pos");
	m_scale = JsonManager::Instance().GetParamVec2(_filePath, "NumThree", "scale");
	m_texSize = JsonManager::Instance().GetParamVec2(_filePath, "NumThree", "texSize");
	m_texSizeHarf = JsonManager::Instance().GetParamVec2(_filePath, "NumThree", "texSizeHarf");
	m_anim = JsonManager::Instance().GetParamVec2(_filePath, "NumThree", "anim");
	m_color = JsonManager::Instance().GetParamVec4(_filePath, "NumThree", "color");
	m_drawFlg = JsonManager::Instance().GetParam<bool>(_filePath, "NumThree", "drawFlg");
	JsonManager::Instance().AddParamVec2("Asset/Data/Json/UI/Text/NumThree.json", "NumThree", "anim", m_anim);
}

void NumThree::DrawSprite()
{
...
void NumThree::Update()
{
		...
		&& nowLength.x >= m_pos.x - m_texSizeHarf.x && nowLength.x <= m_pos.x + m_texSizeHarf.x)
	{
		...
		if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
		{
			if (m_key == false)
			{
				Fade::Instance().BootBlackFade(SceneManager::SceneType::Stage3);
				m_key = true;
			}
		}
		else
